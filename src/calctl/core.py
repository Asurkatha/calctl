...
def _check_conflicts(events: List[Dict], new_event: Event, exclude_id: Optional[str] = None) -> List[Dict]:
        ...
        if conflicts:
            ...
            for c in conflicts:
                ...
                conflict_details.append(
                    f'"{c["title"]}" ({conflict_event.start_time} - {end_time})'
                )
            raise ValueError(f"Event conflicts with {', '.join(conflict_details)}. Use --force to schedule anyway")
...
def list_events(
    *,
    ...
    if today:
        ...
        # Only apply "today onwards" filter if no explicit date filters provided
...
def show_event(event_id: str, *, db_path: Optional[str] = None) -> Optional[Dict[str, Any]]:
...
def delete_event(event_id: str, *, db_path: Optional[str] = None) -> Optional[Dict[str, Any]]:
...
def delete_events_by_date(date: str, *, db_path: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    for event in events:
        if event['id'] == event_id:
            ...
            if description is not None:
                updated_data['description'] = description.strip() if description else None
            ...
            if conflicts:
                ...
                raise ValueError(f"Edit would create conflicts with {', '.join(conflict_details)}")
